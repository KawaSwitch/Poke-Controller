...
- スティック周りの機能追加
  ...
  **スティックの傾きの強さの設定**
  ...
  - r=1.0をデフォルト値としているので\
    ...
    **マウスクリックでスティック操作機能**
  - マウスで直感的な操作ができそうな感じにしています。~~操作はかなり難しいです。~~\
    操作円の半径は変更可能なので、需要があればConfigファイルに載せます。 タッチパネル使用の際などの挙動は不明ですが、そちらのほうが向いているかもしれません。\
...
- メニュー機能の追加
  ...
  - キーコンフィグ追加
    ...
    注意点として、複数キーを同時に割り当てても、同時に入力されることはありません。\
